...
- **后端**: Hono 4.11.1 + Cloudflare Workers
- **前端**: React 19.2.1 + TypeScript 5.8.3 + Vite 6.0.0
- **数据库**: D1 (SQLite)
- **KV存储**: Cloudflare Workers KV
- **对象存储**: Cloudflare R2
- **工具库**: XLSX (Excel处理), Zod (数据验证), React Icons, Ant Design 6.1.3
...
#### 环境初始化
```bash
npm run init       # 初始化本地开发环境 (清除缓存 + 初始化表 + 同步数据)
```

**使用场景**：
- 在 `npm install` 之后首次启动开发环境
- 本地数据出现异常需要重置时
- 需要同步最新的远程数据时
...
npm run wrangler    # 后端开发服务器 (Wrangler，端口 8787) 运行时需要先构建 npm run build 非热更新
...
# Schema 配置管理
npm run kv:apply:local    # 应用本地 kv-schema.json 配置到本地 KV
npm run kv:apply:remote   # 应用本地 kv-schema.json 配置到远程 KV
npm run kv:validate       # 验证 kv-schema.json 配置格式
```
...
项目使用 `kv-schema.json` 文件定义 KV 存储的数据结构，实现前端配置的版本化管理。

**相关文件**：
- `kv-schema.json` - KV 数据结构定义文件
- `scripts/sync-kv.cjs` - KV 同步和验证脚本
...
- ListParts - 列出分片
- PutBucketEncryption - 设置桶加密
- PutBucketCors - 设置桶 CORS
- PutBucketLifecycleConfiguration - 设置桶生命周期配置
- Lifecycle - 生命周期管理操作
- StorageTier - 存储层级操作
- Transition - 转换操作
...
- GetBucketCors - 获取桶 CORS 配置
- GetBucketLifecycleConfiguration - 获取桶生命周期配置

### 开发建议

1. **减少 A 类操作**：避免频繁调用 ListObjects、PutObject 等操作
2. **优先使用 B 类操作**：预览文件使用 GetObject，获取元数据使用 HeadObject
3. **手动触发**：对于 A 类操作（如列表加载），应由用户手动触发，而非自动加载
...
书签管理接口用于前台导航系统的内容管理，数据存储在 KV 中。

**相关文件**：
- `src/worker/controllers/bookmarks.controller.ts` - 书签 API 控制器
- `kv-schema.json` - KV 数据结构定义
- `src/react-app/pages/frontend/bookmarks/` - 前端页面组件

| 方法 | 路径 | 权限 | 说明 |
|------|------|------|------|
| GET | `/api/bookmarks/config` | `frontend:bookmarks:read` | 获取书签配置 |
| PUT | `/api/bookmarks/config` | `frontend:bookmarks:update` | 更新书签配置 |
...
**KV Schema 管理**：
- 配置文件：`kv-schema.json`
- 应用到本地：`npm run kv:apply:local`
- 应用到远程：`npm run kv:apply:remote`
- 验证配置：`npm run kv:validate`

## D1 接口 (权限系统)

### 权限系统架构

项目采用统一的权限常量管理，所有权限定义位于 `src/worker/constants/permissions.ts`。

**权限命名规范**：`模块:子模块:操作`（如 `system:user:read`）

**支持 7 大功能模块**：

| 模块 | 说明 | 权限数量 |
|------|------|---------|
| 系统管理 | 用户、角色、菜单、组织、SQL 查询 | 24 |
| OAuth | 客户端管理、权限组管理 | 10 |
| R2 | 文件管理、文件夹管理 | 6 |
| 内容管理 | 文章、分类（预留） | 8 |
| 前台配置 | Bookmarks 管理 | 4 |
| 用户中心 | 个人信息、密码、通知（预留） | 5 |
| 仪表盘 | 数据查看（预留） | 2 |

**超级管理员权限**：`*:*:*` - 拥有所有权限

**权限通配符匹配**：支持 `system:user:*` 格式的通配符权限
...
| 权限标识 | 说明 |
|---------|------|
| `r2:file:view` | 查看文件列表和详情 |
| `r2:file:manage` | 上传和下载文件 |
| `r2:file:delete` | 删除文件 |
| `r2:folder:view` | 查看文件夹列表 |
| `r2:folder:create` | 创建文件夹 |
| `r2:folder:delete` | 删除文件夹 |

**注意**：权限定义位于 `src/worker/constants/permissions.ts`
...
| `/dashboard/frontend/bookmarks` | BookmarksManage | 书签管理 |
